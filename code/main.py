...
class Game:
    ...
    def __init__(self):
        ...
        self.player_monsters = {
			0: Monster('Sparchu', 10),
			1: Monster('Finsta', 10),
			2: Monster('Plumette', 10),
			# 3: Monster('Atrox', 24),
        ...
        self.tint_direccion = -1
        self.tint_speed = 600


        self.import_assets()
        self.setup(self.tmx_maps['world'], 'house')
        print(self.tmx_maps['world'])
        #self.audio['overworld'].play(-1)

        # Overlays
        self.dialoge_tree = None
    ...
    def input(self):
        ...
        if not self.dialoge_tree and not self.battle:
            #get_just_pressed() → solo realiza la accion cada vez que apreto SPACE
    ...
    def create_dialog(self, character):
        if not self.dialoge_tree:
            self.dialoge_tree = DialogTree(character, self.player, self.all_sprites, self.fonts['dialog'], self.end_dialog)
        ...
        for index, monster in self.player_monsters.items():
            if monster.evolution:
                if monster.level == monster.evolution[1]:
                    ...
                    self.player.block()
                    self.evolution = Evolution(self.monster_frames['monsters'], 
                                               monster.name, monster.evolution[0], 
                                               self.fonts['bold'], 
                                               self.end_evolution,
        ...
        if sprites and self.player.direction:
            ...
            self.transition_target = Battle(
                player_monsters = self.player_monsters, 
                   opponent_monsters = {index: Monster(monster, sprites[0].level + randint(-2, 1)) for index, monster in enumerate(sprites[0].monsters)}, 
                   monster_frames = self.monster_frames, 
    ...
    def run(self):
        while True:
            ...
            # Overlays
            if self.dialoge_tree: self.dialoge_tree.update()
