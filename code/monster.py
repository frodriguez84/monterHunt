...
class Monster:
    def __init__(self, name, level):
        ...
        self.initiative = 0
        self.albilities = MONSTER_DATA[name]['abilities']
    ...
    def get_abilities(self, all = True):
        if all:
            return [ability for lvl, ability in self.albilities.items() if self.level >= lvl]
        else:
            return [ability for lvl, ability in self.albilities.items() if self.level >= lvl and ATTACK_DATA[ability]['cost'] < self.energy]
